...
TOOL_ICONS = {
    ...
    "zbrush":               "fa-solid fa-pen-nib",
    "substance":            "fa-solid fa-palette",
    "substance painter":    "fa-solid fa-palette",
    "substance designer":   "fa-solid fa-palette",
    "pbr":                  "fa-solid fa-layer-group",
    "unreal":               "fa-solid fa-gamepad",
    "unity":                "fa-solid fa-gamepad",
    "maya":                 "fa-solid fa-cubes",
    "3ds max":              "fa-solid fa-cubes",
    "houdini":              "fa-solid fa-wind",
    "marvelous":            "fa-solid fa-shirt",
...
def cat_label(cat: str, mkt: bool = False) -> str:
    lbl = CAT_LABELS_MKT if mkt else CAT_LABELS_PROJ
    return lbl.get(cat, cat.capitalize() if cat else "Other")


def cat_tag_label(category: str) -> str:
    """Exact card-tag label used inside index.html cards."""
    mapping = {
        "environment": "Environment",
        "character":   "Character",
        "prop":        "Props &amp; Assets",
...
def tool_icon(tool: str) -> str:
    """Map a tool name to its Font Awesome icon class."""
    return TOOL_ICONS.get(tool.lower().strip(), "fa-solid fa-wrench")
...
class DataStore:
    ...
    def portfolio_html(self) -> str:
        """
        Generate <article> cards that exactly match the structure
        already in index.html — single-line card-img / card-body,
        correct classes, correct tool icon mapping, correct indentation.
        """
        cards = []
        for p in self.projects:
            category = p.get("category", "environment")
            title    = p.get("title", "")
            desc     = p.get("desc", "")
            icon     = p.get("icon", "fa-solid fa-cube")
            gradient = p.get("gradient", "env-gradient")
            tools    = p.get("tools") or []
            image    = p.get("image")
            cat_tag  = cat_tag_label(category)
            ...
            img_html = (
                ...
                if image
                else f'<div class="img-placeholder"><i class="{icon}"></i></div>'
            )
            card_img = (
                f'<div class="card-img {gradient}">'
                f'{img_html}'
                f'<div class="card-overlay">'
                f'<button class="btn btn-sm">View Work</button>'
                f'</div>'
                f'<div class="card-glow"></div>'
                f'</div>'
            )

            # tool chips
            chips    = "".join(
                f'<span><i class="{tool_icon(t)}"></i> {t}</span>'
                for t in tools
            )
            card_tools = f'<div class="card-tools">{chips}</div>'

            # card-body
            card_body = (
                f'<div class="card-body">'
                f'<span class="card-tag">{cat_tag}</span>'
                f'<h3>{title}</h3>'
                f'<p>{desc}</p>'
                f'{card_tools}'
                f'<a href="#" class="btn btn-outline btn-sm">'
                f'View Details <i class="fa-solid fa-arrow-right"></i>'
                f'</a>'
                f'</div>'
            )

            # full article — indentation matches index.html exactly
            cards.append(
    ...
    def products_js(self) -> str:
        ...
        for i, m in enumerate(self.market):
            ...
            items.append(
                f"  {{\n"
                f"    id:            {m['id']},\n"
                f"    title:         '{esc_js(m['title'])}',\n"
                f"    category:      '{m['category']}',\n"
                f"    desc:          '{esc_js(m['desc'])}',\n"
                f"    price:         {m['price']},\n"
                f"    originalPrice: {js_val(m.get('originalPrice'))},\n"
                f"    rating:        {m['rating']},\n"
                f"    reviews:       {m['reviews']},\n"
                f"    software:      {json.dumps(m.get('software', []))},\n"
                f"    formats:       {json.dumps(m.get('formats', []))},\n"
                f"    image:         {js_val(m.get('image'))},\n"
                f"    icon:          '{esc_js(m['icon'])}',\n"
                f"    badges:        {json.dumps(m.get('badges', []))},\n"
                f"    downloads:     {m['downloads']},\n"
                f"    featured:      {'true' if m['featured'] else 'false'},\n"
                f"    dateAdded:     '{m['dateAdded']}',\n"
                f"  }}{comma}"
            )
    ...
    def patch_index(self, original: str) -> tuple[str, str]:
        """
        Replace everything between the opening and closing tags of
            <div class="projects-grid" id="portfolioGrid"> ... </div>

        Uses a depth-counting string walker — no regex — so nested
        divs inside the cards never confuse the matcher.
        """
        OPEN_TAG     = '<div class="projects-grid" id="portfolioGrid">'
        OPEN_TAG_ALT = '<div id="portfolioGrid" class="projects-grid">'

        start    = original.find(OPEN_TAG)
        open_tag = OPEN_TAG
        if start == -1:
            start    = original.find(OPEN_TAG_ALT)
            open_tag = OPEN_TAG_ALT

        if start == -1:
            # Last resort — insert before closing </section>
            cards   = self.portfolio_html()
            patched = original.replace(
                "</section>",
                f"<!-- CREVIZ: paste cards into your projects-grid div -->\n{cards}\n</section>",
                1,
            )
            return patched, "⚠ Could not find portfolioGrid — inserted before first </section>."
        ...
        depth    = 1
        pos      = content_start
        html_len = len(original)

        while pos < html_len and depth > 0:
            next_open  = original.find("<div",   pos)
            next_close = original.find("</div>", pos)

            if next_close == -1:
                return original, "⚠ Could not find closing </div> for portfolioGrid — file unchanged."
            ...
            if next_open != -1 and next_open < next_close:
                ...
                depth -= 1
                if depth == 0:
                    content_end = next_close  # points to '<' of the matching </div>
                    break
                pos = next_close + 6          # skip past "</div>"
        else:
            return original, "⚠ Unbalanced <div> tags near portfolioGrid — file unchanged."

        cards       = self.portfolio_html()
        new_content = f"\n{cards}\n        "   # 8-space indent before closing </div>
        patched     = original[:content_start] + new_content + original[content_end:]
        return patched, 'Replaced contents of <div id="portfolioGrid"> ✓'

    # ── marketplace.js patcher ────────────────────────────────────
    def patch_marketplace_js(self, original: str) -> tuple[str, str]:
        ...
        patched = (
            f"/* CREVIZ ADMIN: inserted — remove any existing duplicate below */\n"
            f"{products}\n\n{original}"
        )
...
def styled_btn(parent, text, cmd, color=FIRE_ORANGE, fg="#fff",
    ...
    kw = dict(
        text=text, command=cmd, fg=fg, bg=color,
...
class ProjectDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(
            ...
            bg=BG_MID, fg=TEXT_MAIN, font=("Helvetica", 13, "bold"),
        ...
        ir = tk.Frame(inner, bg=BG_DARK)
        ir.pack(fill="x", **pad)
        self.v_image = tk.StringVar()
        entry_widget(ir, self.v_image, width=44).pack(side="left", fill="x", expand=True)
        styled_btn(ir, "Browse", self._browse_image, color=BG_CARD, small=True).pack(
            side="left", padx=(6, 0))
        ...
        styled_btn(foot, "Cancel", self.destroy, color=BG_CARD, fg=TEXT_MUTED, small=True).pack(
            side="right", padx=(0, 12))
        styled_btn(foot, "💾  Save Project", self._save, small=True).pack(
            side="right", padx=(0, 8))
...
class MarketDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(
            hdr,
            text="✏  Edit Asset" if self.asset else "＋  Add New Asset",
            bg=BG_MID, fg=TEXT_MAIN, font=("Helvetica", 13, "bold"),
        ).pack(padx=20, side="left")
        ...
        styled_btn(irow, "Browse", self._browse_image, color=BG_CARD, small=True).pack(
            side="left", padx=(6, 0))
        ...
        styled_btn(foot, "Cancel", self.destroy, color=BG_CARD, fg=TEXT_MUTED, small=True).pack(
            side="right", padx=(0, 12))
        styled_btn(foot, "💾  Save Asset", self._save, small=True).pack(
            side="right", padx=(0, 8))
    ...
    def _save(self):
        ...
        desc  = self.w_desc.get("1.0", "end").strip()
        if not title:
            messagebox.showerror("Validation", "Asset title is required.", parent=self)
            return
        if not desc:
            messagebox.showerror("Validation", "Description is required.", parent=self)
            return
...
class CardRow(tk.Frame):
    ...
    def _build(self, item, on_edit, on_delete, on_duplicate, is_market):
        ...
        tk.Label(
            top,
            text=f"  {cat_label(item.get('category', ''), mkt=is_market)}",
            bg=BG_CARD, fg=FIRE_ORANGE, font=("Helvetica", 8, "bold"),
        ).pack(side="left", padx=6)
        if is_market:
            price = item.get("price", 0)
            tk.Label(
                top,
                text="Free" if price == 0 else f"₹{price:,}",
...
class CommitDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(
            ...
            bg=BG_DARK, fg=TEXT_MUTED, font=("Helvetica", 9),
        ...
        presets.pack(padx=24, anchor="w")
        tk.Label(presets, text="Quick:", bg=BG_DARK, fg=TEXT_MUTED,
                 font=("Helvetica", 8)).pack(side="left", padx=(0, 6))
        for label in ["feat: update portfolio", "fix: bug fix",
                      "chore: update assets", "style: UI improvements"]:
            styled_btn(
                presets, label,
        ...
        styled_btn(foot, "Cancel", self.destroy, color=BG_CARD, fg=TEXT_MUTED, small=True).pack(
            side="right", padx=(0, 12))
        styled_btn(foot, "✔  Commit", self._commit, color=FIRE_ORANGE, small=True).pack(
            side="right", padx=(0, 8))

    def _commit(self):
        msg = self.msg_var.get().strip()
        if not msg:
            messagebox.showerror("Empty message", "Please enter a commit message.", parent=self)
            return
        self.destroy()
        self.on_confirm(msg)
...
class App(tk.Tk):
    ...
    def _build_header(self):
        ...
        styled_btn(right, "💾  Save & Export", self._save_and_export, small=True).pack(
            side="right", padx=(6, 0))
    ...
    def _build_main(self, parent):
        ...
        for name in ["dashboard", "portfolio", "marketplace", "export", "git", "settings"]:
            self._panels[name] = tk.Frame(self._main, bg=BG_DARK)
    ...
    def _build_dashboard(self, parent):
        ...
        styled_btn(btns, "🔀  Git & Deploy",
                   lambda: self._switch_panel("git"), color=PURPLE, fg="#fff",
                   ).pack(side="left")
    ...
    def _refresh_projects(self):
        ...
        if not items:
            tk.Label(self._proj_list, text="No projects found.",
                     bg=BG_DARK, fg=TEXT_MUTED, font=("Helvetica", 11)).pack(pady=40)
            return
        for p in items:
            CardRow(
                self._proj_list, p,
    ...
    def _build_export(self, parent):
        ...
        self._build_patch_section(
            ...
            'Upload your current index.html — replaces <div id="portfolioGrid"> contents automatically.',
            "📂  Upload index.html",
            [("HTML", "*.html *.htm"), ("All", "*.*")],
            "_index_content", "_index_name", "_index_status_lbl",
            "⬇  Download Patched index.html",
            self._patch_and_download_index,
            "_index_dl_btn",
            FIRE_ORANGE,
        )
        tk.Frame(inner, bg=BORDER, height=1).pack(fill="x", pady=16)
        self._build_patch_section(
            ...
            "📂  Upload marketplace.js",
            [("JS", "*.js"), ("All", "*.*")],
            "_js_content", "_js_name", "_js_status_lbl",
            "⬇  Download Patched marketplace.js",
            self._patch_and_download_js,
            "_js_dl_btn",
            FIRE_YELLOW,
        )
    ...
    def _build_export_section(self, parent, title, subtitle, code_attr):
        ...
        cr = tk.Frame(frame, bg=BG_MID)
        cr.pack(fill="x", padx=8, pady=(6, 0))
        ...
        ct = tk.Text(
            frame, height=10, bg="#0a0806", fg="#f0ece8",
    ...
    def _patch_and_download_index(self):
        if not self._index_content:
            messagebox.showerror("No file", "Please upload index.html first.")
            return
    ...
    def _patch_and_download_js(self):
        if not self._js_content:
            messagebox.showerror("No file", "Please upload marketplace.js first.")
            return
    ...
    def _build_git(self, parent):
        hdr = tk.Frame(parent, bg=BG_DARK, pady=14)
        hdr.pack(fill="x", padx=24)
        tk.Label(hdr, text="🔀  Git & Firebase Deploy", bg=BG_DARK, fg=TEXT_MAIN,
                 font=("Helvetica", 15, "bold")).pack(side="left")
        ...
        self._make_git_btn(row3, "🚀  firebase deploy",
                           color="#1a1a2a", fg=FIRE_YELLOW,
                           cmd=self._firebase_deploy,
                           confirm="Deploy to Firebase Hosting?\nThis will publish your site live.",
                           width=22,
                           ).pack(side="left", padx=(0, 8))
        self._make_git_btn(row3, "📋  firebase hosting:channel:list",
                           color="#1a1a2a", fg=FIRE_YELLOW,
                           cmd=lambda: self._run_git(
                               self._firebase_cmd("hosting:channel:list")),
                           ).pack(side="left")
        ...
        # Terminal
        tk.Label(parent, text="🖥  Terminal Output", bg=BG_DARK, fg=TEXT_MAIN,
        ...
        tbar = tk.Frame(term_frame, bg="#0a0806")
        tbar.pack(fill="x")
        ...
        vsb = tk.Scrollbar(term_frame, orient="vertical", command=self._terminal.yview)
        self._terminal.configure(yscrollcommand=vsb.set)
        vsb.pack(side="right", fill="y")
        self._terminal.pack(fill="both", expand=True)

        self._terminal.tag_configure("cmd",     foreground=FIRE_ORANGE,
        ...
        self._terminal.tag_configure("section", foreground=PURPLE,
                                     font=("Courier New", 10, "bold"))
    ...
    def _make_git_btn(self, parent, text, cmd, color, fg,
        ...
        def wrapped():
            ...
            cmd()
        kw = dict(
            text=text, command=wrapped, fg=fg, bg=color,
            activeforeground=fg, activebackground=color,
            relief="flat", bd=0, cursor="hand2",
            font=("Helvetica", 10, "bold"),
            padx=18, pady=10,
    ...
    def _terminal_write(self, text: str, tag: str = ""):
        ...
        self._terminal.insert("end", text, tag) if tag else self._terminal.insert("end", text)
    ...
    def _run_git(self, cmd: list, cwd: str = None, on_done=None, silent: bool = False):
        """
        Run any shell command in a background thread and stream
        output live to the terminal widget.

        Windows fix:
          - shell=True  lets cmd.exe find npm-installed .cmd wrappers
                        (firebase.cmd, npm.cmd, etc.)
          - cmd_arg is joined to a string when shell=True
          - os.environ.copy() ensures the full system PATH is inherited
          - CREATE_NO_WINDOW hides the black console popup on Windows
        """
        ...
        def worker():
            try:
                ...
                msg = (
                    ...
                    f"    — make sure '{cmd[0]}' works in a new CMD window first,\n"
                    f"    then restart this app so it picks up the updated PATH.\n"
                )
                self.after(0, self._terminal_write, msg, "err")
                self.after(0, self._set_git_status, f"● '{cmd[0]}' not found", RED_ERR)
                if on_done:
    ...
    # ── Firebase command resolver ─────────────────────────────────
    def _firebase_cmd(self, *args) -> list:
        """
        Resolve the correct firebase executable for the current OS.

        On Windows: firebase is installed by npm as firebase.cmd
                    Use 'where firebase' to find the exact .cmd path.
                    Falls back to plain 'firebase' (shell=True handles it).
        On mac/linux: plain 'firebase' on PATH works fine.
        """
    ...
    def _git_auto_status(self):
        """Auto-run git status when the Git panel is opened."""
        self._terminal_write("\n─── Opened Git Panel ───\n", "section")
    ...
    def _firebase_deploy(self):
        ...
        self._run_git(self._firebase_cmd("deploy"))
    ...
    def _full_workflow(self):
        ...
        Each step waits for the previous; chain stops on any failure.
        """
        ...
        self._terminal_write("\n══════════════════════════════════════\n", "section")
        self._terminal_write("⚡  FULL WORKFLOW — Add → Commit → Push → Deploy\n", "section")
        self._terminal_write("══════════════════════════════════════\n", "section")
        self._terminal_write("\n─── Step 1 / 4 — git add . ───\n", "section")
    ...
    def _build_settings(self, parent):
        ...
        tk.Label(
            pf,
            text=(
                "Use the  🔀 Git & Deploy  panel to run git commands and firebase deploy.\n"
                "Set your project folder path there before running any commands."
            ),
            bg=BG_CARD, fg=TEXT_MUTED, font=("Helvetica", 9), justify="left",
        ).pack(anchor="w")
        ...
        for line in [
            "Creviz Studio — Desktop Admin Panel  v1.1",
    ...
    def _confirm_reset(self):
        if messagebox.askyesno(
            "Reset All Data",
            "This will permanently delete all your projects and marketplace assets\n"
            "and restore the built-in defaults.\n\nThis cannot be undone. Continue?",
            icon="warning",
        ):
            self.store.reset()
            self._refresh_all()
