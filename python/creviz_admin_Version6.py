...
class App(tk.Tk):
    ...
    def _build_git(self, parent):
        ...
        self._make_git_btn(row3, "📋  firebase hosting:channel:list",
                           color="#1a1a2a", fg=FIRE_YELLOW,
                           cmd=lambda: self._run_git(["firebase","hosting:channel:list"])
                           ).pack(side="left")
        ...
        def worker():
            try:
                ...
                for line in proc.stdout:
                    ...
                    # Colour-code lines
                    lower = line.lower()
                    if any(w in lower for w in ("error","fatal","failed","conflict")):
                        tag = "err"
                    elif any(w in lower for w in ("warning","warn")):
                        tag = "warning"
                    elif any(w in lower for w in ("success","complete","done",
                                                   "deployed","pushed","up-to-date")):
                        tag = "ok"
                    else:
                        tag = ""
                ...
                if rc == 0:
                    if not silent:
                        self.after(0, self._terminal_write,
                                   f"✔ Command completed successfully (exit 0)\n", "ok")
                ...
                if on_done:
                    self.after(0, on_done, rc, "".join(output_lines))

            except FileNotFoundError:
                msg = (f"✖ '{cmd[0]}' not found.\n"
                       f"  Make sure {cmd[0]} is installed and on your PATH.\n")
    ...
    def _git_auto_status(self):
        """Run git status silently when Git panel is opened."""
    ...
    def _firebase_deploy(self):
        ...
        self._run_git(["firebase","deploy"])

    def _full_workflow(self):
        """
        Sequential pipeline:
        1. git add .
        2. CommitDialog → git commit -m "..."
        3. git push
        4. firebase deploy
        """
        def step3_push(rc, _):
            if rc != 0:
                self._terminal_write("⚠ Push failed — skipping firebase deploy.\n","warning")
                return
            self._terminal_write("\n─── Step 4 / 4 — Firebase Deploy ───\n","section")
            self._run_git(["firebase","deploy"])

        def step2_push_after_commit(msg: str):
            def step_push(rc, _):
                if rc != 0:
                    self._terminal_write("⚠ Commit failed — skipping push & deploy.\n","warning")
                    return
                self._terminal_write("\n─── Step 3 / 4 — git push ───\n","section")
                self._run_git(["git","push"], on_done=step3_push)
            self._run_git(["git","commit","-m", msg], on_done=step_push)

        def after_add(rc, _):
            if rc != 0:
                self._terminal_write("⚠ git add failed — aborting workflow.\n","warning")
                return
            self._terminal_write("\n─── Step 2 / 4 — git commit ───\n","section")
            CommitDialog(self, on_confirm=step2_push_after_commit)
